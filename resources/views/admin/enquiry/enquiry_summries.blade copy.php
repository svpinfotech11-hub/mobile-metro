...
<section class="content">
            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        @php
                            // Decode products and calculate total CFT
                            $products = json_decode($enquiry->products_item, true);
                            $totalCft = 0;
                    
                            if (is_array($products)) {
                                foreach ($products as $product) {
                                    $totalCft += $product['total_cft'];
                                }
                            }
                        @endphp
                        ...
                        <p><strong>Payment Date:</strong>
                            ...
                            <div class="section">
                                ...
                                <!--       </thead>-->
                                <!--       <tbody>-->
                                <!--           @if(!empty($enquiry->items) && is_array($enquiry->items))-->
                                <!--           @foreach($enquiry->items as $item)-->
                                <!--           <tr>-->
                                <!--               <td>{{ $item['product_name'] }}</td>-->
                                <!--               <td>{{ $item['quantity'] }}</td>-->
                                <!--               <td>-->
                                <!--                   {{ number_format($item['product_cft']) }}-->
                                ...
                                <!--               <td>Total CFT:{{ number_format($enquiry->total_cft) }}</td>-->
                                <!--               <td style="text-align:right;">Total : {{ number_format($enquiry->total_amount) }}</td>-->
                                <!--               <td colspan="4" style="text-align:right;">Grand Total: {{ number_format($enquiry->grand_total_cost) }}</td>-->
                                <!--           </tr>-->
                                <!--           @endif-->
                                <!--       </tbody>-->
                                <!--   </table>-->
                                <!-- <table class="enquiry-summary">
                                    <thead>
                                        <tr>
                                            <th>Item Name</th>
                                            <th>Quantity</th>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ number_format($item['total_cft']) }}</td>
                                            <td>{{ ucfirst($item['rate_type']) }}</td>
                                            <td>{{ number_format($item['cft_rate']) }}</td>
                                            <td>{{ number_format($item['amount']) }}</td>
                                            <td>{{ number_format($item['cft_profit']) }}</td>
                                            <td>{{ number_format($item['total_cost']) }}</td>
                                        </tr>
                                        @endforeach
                                        ...
                                        <tr class="totals-row">
                                            ...
                                            <td>{{ number_format($enquiry->total_cft) }}</td>
                                            <td colspan="2" style="text-align:right;">Total Amount:</td>
                                            <td>{{ number_format($enquiry->total_amount) }}</td>
                                            <td style="text-align:right;">Product Total Cost:</td>
                                            <td>{{ number_format($enquiry->grand_total_cost - $enquiry->total_km_cost) }}</td>
                                        </tr>

                                        <tr class="section-header">
                                            <td colspan="9">Distance Based Charges</td>
                                        </tr>

                                        <tr>
                                            <td colspan="4" style="text-align:right;">Distance (KM):</td>
                                            <td colspan="5">{{ number_format($enquiry->km_distance) }} km</td>
                                        </tr>
                                        <tr>
                                            <td colspan="4" style="text-align:right;">Rate Type:</td>
                                            <td colspan="5">{{ ucfirst($enquiry->km_rate_type) }}</td>
                                        </tr>
                                        <tr>
                                            <td colspan="4" style="text-align:right;">KM Rate:</td>
                                            <td colspan="5">{{ number_format($enquiry->km_rate) }}</td>
                                        </tr>
                                        <tr>
                                            <td colspan="4" style="text-align:right;">KM Profit:</td>
                                            <td colspan="5">{{ number_format($enquiry->km_profit) }}</td>
                                        </tr>
                                        <tr class="totals-row">
                                            <td colspan="4" style="text-align:right;">Total KM Cost:</td>
                                            <td colspan="5">{{ number_format($enquiry->total_km_cost) }}</td>
                                        </tr>

                                       <tr class="grand-total-row">
                                            <td colspan="8" style="text-align:right;">Grand Total (Products + KM):</td>
                                            <td>{{ number_format($enquiry->grand_total_cost) }}</td>
                                        </tr>
                                    </tbody>
                                </table> -->
    ...
    if (!empty($enquiry->items)) {
        foreach ($enquiry->items as $item) {
            if (!empty($item['rate_type'])) {
                $rateType = strtoupper($item['rate_type']);
                $cftRate = $item['cft_rate'] ?? 0;
                $cftProfit = $item['cft_profit'] ?? 0;
                break; // first valid item is enough
            }
        }
    }

    $totalCftCost = $enquiry->grand_total_cost - $enquiry->total_km_cost;
@endphp
...
<table class="enquiry-summary charges-table">
    ...
    <tr>
        <td colspan="3" style="text-align:right;">TOTAL CFT :</td>
        <td colspan="3">{{ number_format($enquiry->total_cft, 2) }}</td>
    </tr>
    ...
    <tr>
        ...
        <td colspan="3">{{ number_format($cftRate, 2) }}</td>
    </tr>

    <tr>
        <td colspan="3" style="text-align:right;">CFT PROFIT :</td>
        <td colspan="3">{{ number_format($cftProfit, 2) }}</td>
    </tr>
    ...
    <tr class="totals-row">
        ...
        <td colspan="3">{{ number_format($totalCftCost, 2) }}</td>
    </tr>
